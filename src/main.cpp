...
#if defined(NDEBUG)
# error "blackholecoin cannot be compiled without assertions."
#endif

#define ZEROCOIN_MODULUS   ""
...
uint256 hashGenesisBlock = uint256("0x563ac70cc2642286ad8463559011621fc4debe7ab2525900f74d079fc73cb5f2");
uint256 hashGenesisMerkleRoot = uint256("0x2c8a40ec8bf2431bb2cc23c330ca63ef12c17ed474a86ef277c4c3c70f327c80");
...
int64 CTransaction::nMinTxFee = 100000; // 0.001 blackholecoin
/** Fees smaller than this (in ztoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 100000; // 0.001 blackholecoin
...
const string strMessageMagic = "blackholecoin Signed Message:\n";
...
{
    // blackholecoin: IsDust() detection disabled, allows any valid dust to be relayed.
                                ...
                                if (passVerify) {
                                    ...
                                    if (!isVerifyDB) {
                                        ...
                                        if (!isAlreadyStored) {
                                            ...
                                            if(nHeight > 8500 && nHeight < INT_MAX){
...
static const int64 nTargetTimespan = 600; // 2,5 minutes between retargets
static const int64 nTargetSpacing = 600; // 2,5 minutes blocks
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot, bool isVerifyDB) const
{
    ...
    // blackholecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // blackholecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "new blackholecoin";
        CTransaction txNew;
       ...
       // uint256 hash = block.GetHash();
        ...
        assert(block.hashMerkleRoot == uint256("0x2c8a40ec8bf2431bb2cc23c330ca63ef12c17ed474a86ef277c4c3c70f327c80"));
        block.print();
        assert(hash == hashGenesisBlock);
...
unsigned char pchMessageStart[4] = { 0xee, 0xd9, 0xf3, 0xf1 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("blackholecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("blackholecoinMiner : ProcessBlock, block not accepted");
    }
...
void static blackholecoinMiner(CWallet *pwallet)
{
    printf("blackholecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blackholecoin-Miner");
    ...
    try { while(true) {
        ...
        printf("Running blackholecoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                LYRA2(BEGIN(thash), 32, BEGIN(pblock->nVersion), 80, BEGIN(pblock->nVersion), 80, 2, 330, 256);
    ...
    {
        printf("blackholecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&blackholecoinMiner, pwallet));
}
